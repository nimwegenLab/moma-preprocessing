...
#     def get_test_configs__for__test__normalize_image(self):
#         tests = []
...
#         return tests
#
#     def test__normalize_image(self):
...
#
#         tests = self.get_test_configs__for__test__normalize_image()
#
#         for test in tests:
#             path = test['path']
    ...
    def test__normalize_image(self):
        ...
        for test in [tests[0]]:
            ...
            with self.subTest(test=test['name']):
                ...
                imageProcessor.load_numpy_image_array(imdata)
                imageProcessor.growthlane_length_threshold = growthlane_length_threshold
                imageProcessor.main_channel_angle = rotation_angle
                imageProcessor.process_image()
                # channel_centers = imageProcessor.channel_centers

                imageProcessor.determine_image_shift(imdata)

                plt.imshow(imdata)
                plt.show()

                imageProcessor.get_normalization_ranges(imdata, box_pts=11)
                print('Finished')
