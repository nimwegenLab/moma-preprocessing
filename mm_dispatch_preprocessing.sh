...
#  if [ -n "$FIJI_MACRO" ]; then CMD_STR="$CMD_STR -m $FIJI_MACRO"; fi # append optional argument
  ...
  if [ -n "$INTENSITY_NORMALIZATION_RANGE_CUTOFFS" ]; then CMD_STR="$CMD_STR --intensity_normalization_range_cutoffs \"$FORCED_INTENSITY_NORMALIZATION_RANGE\""; fi # append optional argument
